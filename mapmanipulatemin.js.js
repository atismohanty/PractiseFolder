var data=[];var markerContent=[];var fileAdd;var fmData;var mapadd;var oppType=["opportunity","tagstore","estimate","alarm"];getLocation().then(function(res)
{console.log(res)}).catch(function(err)
{console.log(err)})
if(navigator.onLine==!1)
{document.getElementById("mapviewer").text="System is offline. Not able to load the map.";alert}
function initMap(info,paramlat,paramlng,paramzoom)
{var text=info;if(text=="undefined"||text=="NULL")
{document.getElementById("mapviewer").innerHTML="There is no place to show in the map";alert}
var arr=text.split('=');fileAdd=arr[0];for(var i=0;i<arr.length-1;i++)
{var arr2=arr[i+1].split('~');data[i]={"custid":arr2[0],"custname":arr2[1],"lng":arr2[3],"lat":arr2[2],"custtype":arr2[4],"custAdd":arr2[5],"custCity":arr2[6],"custState":arr2[7],"storeContact":arr2[8],"flag":arr2[9],}}
for(var i=0;i<data.length;i++)
{markerContent[i]=data[i].custname}
var mapObj=document.getElementById("mapviewer");var latlng=new google.maps.LatLng(paramlat||data[0].lat,paramlng||data[0].lng);var mapOpt={center:latlng,zoom:parseInt(paramzoom)||1,mapTypeControl:!1};mapadd=new google.maps.Map(mapObj,mapOpt);mapadd.setCenter(latlng);var filePath=fileAdd;var iconImg=[];var marker=[];var infowindow=[];for(var i=0;i<data.length;i++)
...
{}}
fmData=address+'~'+state+'~'+city+'~'+country+'~'+postcode+'~'+placeId+'~'+lat+'~'+lng+'~'+mapadd.getZoom();createNewCustomer()}}})})}
function navigateCustomer(custid)
{var scriptFM="fmp://$/GasketApp.fmp12?script=NavigateCustomer_TriggerJS&param="+custid;window.location.href=scriptFM}
function createNewCustomer()
{var userOpt=confirm("Create a new customer for the selected location?");if(userOpt==!0)
{var scriptFM="fmp://$/GasketApp.fmp12?script=CreateNewCustomerWeb_TriggerJS&param="+fmData;window.location.href=scriptFM}}
function NavCust(id)
{setTimeout(navigateCustomer(id),2000)}
function changeStatus(opps,custid,lat,lng)
{if(opps==1)opps="opportunity";else if(opps==2)opps="tagstore";else if(opps==3)opps="estimate";else if(opps==4)opps="alarm";var paramfm=opps+"~"+custid+"~"+lat+"~"+lng;var scriptFM="fmp://$/GasketApp.fmp12?script=UpdateCustomer_TriggerJS&param="+paramfm;window.location.href=scriptFM}
function getLocation()
{return new Promise(function(resolve,reject)
{var xhttp=new XMLHttpRequest();xhttp.open("POST","https://www.googleapis.com/geolocation/v1/geolocate?key=AIzaSyC-vp1aHmSABBDCKkCxAXoTUWO3atMKXPU",!0);xhttp.onload=function()
{if(this.readyState==4&&this.status==200)
{resolve(this.response)}
else{reject({'status':this.status,'response':this.statusText})}};xhttp.onerror=function()
{reject({'status':this.status,'response':this.statusText})}
xhttp.send()})}